document.addEventListener('DOMContentLoaded', async function () {
    try {
        await fetchAndDisplayEntryLogs('Big Slide')
        ...
        ndef.addEventListener('reading', async ({ message }) => {
            if (message.records.length > 0) {
                ...
                const activityCoins = await getCoinsForActivityFromFirebase('Big Slide');
                ...
                if (coinsWallet >= activityCoins) {
                    ...
                    await storeEntryLogInFirebase(studentName, 'Big Slide');
        ...
        const initialCoinsRequired = await getCoinsForActivityFromFirebase('Big Slide');
        ...
        const entryLogsRef = firebase.database().ref('entryLogs/Big Slide');
        entryLogsRef.on('value', snapshot => {
            const entryLogs = snapshot.val();
            if (entryLogs) {
                
                fetchAndDisplayEntryLogs('Big Slide');
            }
        });
    } catch (error) {
