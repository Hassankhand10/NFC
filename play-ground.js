document.addEventListener('DOMContentLoaded', async function () {
    try {
        await fetchAndDisplayEntryLogs('Play Ground')
        ...
        ndef.addEventListener('reading', async ({ message }) => {
            if (message.records.length > 0) {
                ...
                if (coinsWallet >= activityCoins) {
                    ...
                    await storeEntryLogInFirebase(studentName, "Play Ground"); 
        ...
        const entryLogsRef = firebase.database().ref('entryLogs/Play ground');
        entryLogsRef.on('value', snapshot => {
            const entryLogs = snapshot.val();
            if (entryLogs) {
                
                fetchAndDisplayEntryLogs('Play ground');
            }
        });
    } catch (error) {
